function instantiateTemplate(template, values) {
  ...
  function attr(node, attrName) {
    return (
      node.nodeType == document.ELEMENT_NODE && node.getAttribute(attrName)
    );
  }
    ...
    if (node.nodeType == document.ELEMENT_NODE) {
      ...
      for (var i = 0; i < node.childNodes.length; i++) {
        ...
        var when = attr(child, 'template-when'),
          unless = attr(child, 'template-unless');
        if ((when && !values[when]) || (unless && values[unless])) continue;
...
function test(template, values, expected) {
  ...
  if (result != expected)
    console.log(
      'Unexpected instantiation. Expected\n  ' + expected + '\ngot\n  ' + result
    );
}

test(
  '<h1 template-when="header">{{header}}</h1>',
  { header: 'One' },
  '<h1 template-when="header">One</h1>'
);

test('<h1 template-when="header">{{header}}</h1>', { header: false }, '');

test(
  '<p><img src="icon.png" template-unless="noicon">Hello</p>',
  { noicon: true },
  '<p>Hello</p>'
);

test(
  '<ol><li template-repeat="items">{{name}}: {{score}}</li></ol>',
  {
    items: [
      { name: 'Alice', score: '10' },
      { name: 'Bob', score: '7' },
